...
def train_pytorch_model_from_ApacheParquet(
    ...
    loss_function = (
        getattr(torch, loss_function_name, None)
        or getattr(torch.nn, loss_function_name, None)
        or getattr(torch.nn.functional, loss_function_name, None)
    )
    ...
    class CsvDataset(torch.utils.data.Dataset):
        ...
        def __init__(
            ...
            file_path: str,
            label_column_name: str,
            drop_nan_columns_or_rows: str = "columns",
        ):
            dataframe = pandas.read_parquet(file_path)
        ...
        def __getitem__(self, index):
            return [
                self.features.loc[index].to_numpy(dtype="float32"),
                self.labels.loc[index].to_numpy(dtype="float32"),
            ]
    ...
    for epoch in range(1, number_of_epochs + 1):
        ...
        print(
            f"Training epoch {epoch} completed. Last full batch loss: {last_full_batch_loss:.6f}"
        )
...
if __name__ == "__main__":
    train_pytorch_model_from_ApacheParquet_op = create_component_from_func(
        train_pytorch_model_from_ApacheParquet,
