...
def optimizer_pipeline():
    ...
    # We launch several optimization stages sequentially.
    # At each stage the optimizer suggests several parameter sets to explore based on the available measurements.
    # Each stage depends on the completion of all trials in the previous stage (since only completed trials affect new trial suggestions).
    ...
    for stage in range(optimization_stages):
        ...
        for suggestion_index in range(suggestions_per_stage):
        ...
        new_list_of_metrics_for_parameter_sets = create_list_from_dicts_op(
            *new_metrics_for_parameter_sets
        ).output
        # Collecting metrics for all stages
        all_metrics_for_parameter_sets = combine_lists_op(
            all_metrics_for_parameter_sets, new_list_of_metrics_for_parameter_sets
        ).output
